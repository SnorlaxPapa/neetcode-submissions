...
class Solution:
    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def search(node, parents, pos, parent_node, string):
            nonlocal board
            nonlocal results
            row, col = pos
            ...
            if row - 1 >= 0 and (row - 1, col) not in parents:
                search(node, parents, (row - 1, col), parent_node, string)
            if row + 1 < len(board) and (row + 1, col) not in parents:
                search(node, parents, (row + 1, col), parent_node, string)
            if col - 1 >= 0 and (row, col - 1) not in parents:
                search(node, parents, (row, col - 1), parent_node, string)
            ...
            if not node.children:
                del parent_node.children[board[row][col]]
        
        for row in range (len(board)):
            for col in range (len(board[0])):
                if board[row][col] in word_trie.root.children:
                    search(word_trie.root, set(), (row, col), word_trie.root, "")
