...
class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        for followee in followees:
            followee_tweets = self.users[followee][1]
            followee_tweets_len = len(followee_tweets)
            tweets_extracted = max(-10, -1 * followee_tweets_len)

            for tweet in followee_tweets[tweets_extracted: ]:
