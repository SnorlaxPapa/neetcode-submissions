    ...
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        ...
        for temperature in reversed(temperatures[:-1]):
            ...
            for greater in reversed(greaterTemp):
                if temperature >= temperatures[greater]:
                    greaterTemp.pop()
                    counter+=1
                    print("yo ", counter)
                elif temperature < temperatures[greater]:
                    print("hello!")
                    greaterTemp.append(position)
                    results[position] = greater - position
                    print("final: ", counter)
                    break

            if len(greaterTemp) == 0: #means no greater temperature exists:
                print("hi!")
                results[position] = 0
                greaterTemp.append(position)
